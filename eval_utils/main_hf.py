...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--model_type", "-mt", type=str, default="hf", help="Name of model e.g. `hf`"
    )
    parser.add_argument(
        "--tasks",
        "-t",
...
def cli_evaluate(args: Union[argparse.Namespace, None] = None) -> None:
    ...
    eval_utils.utils.setup_logging(args.verbosity)
    ...
    if isinstance(args.model, str):
        if args.model_args is None:
            ...
            eval_logger.info(
                f"Initializing {args.model} model, with arguments: {simple_parse_args_string(args.model_args)}"
            )
            lm = eval_utils.api.registry.get_model(args.model).create_from_arg_string(
                args.model_args,
                {
