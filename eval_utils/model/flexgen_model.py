...
# print(sys.path)
from eval_utils import utils
from eval_utils.api.instance import Instance
from eval_utils.api.model import TemplateLM
from eval_utils.api.registry import register_model
from eval_utils.model.utils import (
    Collator,
    clear_torch_cache,
    configure_pad_token,
    get_dtype,
...
@register_model("flexgen_model", "flexgen_model-cxl", "flexgen_model_cxl", "flexgen_model-gpu", "flexgen_model_gpu")
class flexgen_model(TemplateLM):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if str(args.gpu_batch_size).startswith("auto"):
            batch_size = args.gpu_batch_size.split(":")
            self.batch_size_per_gpu = batch_size[0]
            self.batch_schedule = float(batch_size[1]) if len(batch_size) > 1 else 1
        else:
            self.batch_size_per_gpu = int(args.gpu_batch_size)
        
        # set up the configure to get OptLM instance
        self.num_prompts = args.num_gpu_batches * args.gpu_batch_size
        self.prompt_len, self.gen_len, self.cut_gen_len = args.prompt_len, args.gen_len, args.cut_gen_len
        gpu = TorchDevice("cuda:0")
        cpu = TorchDevice("cpu")
        disk = TorchDisk(args.offload_dir)
        numa = TorchNuma()
        self.env = ExecutionEnv(gpu=gpu, cpu=cpu, disk=disk, numa=numa, mixed=TorchMixedDevice([gpu, cpu, disk]))

        self.policy = Policy(args.gpu_batch_size, args.num_gpu_batches,
        ...
        self.model_config, self.path, self.weight_map = AutoFlexModel.from_pretrained(args.path)
        self.model_config: FlexModelConfig
        self.path: str
        self.weight_map: dict
        cache_size = self.model_config.cache_bytes(self.num_prompts, self.prompt_len + self.gen_len)
        hidden_size = self.model_config.hidden_bytes(self.num_prompts, self.prompt_len + self.gen_len)
        print(f"If num_prompts: {self.num_prompts}, prompt_len: {self.prompt_len}, gen_len: {self.gen_len}, cut_gen_len: {self.cut_gen_len}"
            f"model size: {self.model_config.model_bytes()/GB:.3f} GB, "
            f"cache size: {cache_size/GB:.3f} GB, "
            f"hidden size (prefill): {hidden_size/GB:.3f} GB")

        print("init weight...")
        self._model = get_model_architecture(config=self.model_config, 
                                             path=self.path, 
                                             policy=self.policy, 
                                             env=self.env, 
                                             weight_map=self.weight_map)
        # self._model = OptLM(config=self.opt_config, 
        #                     env=self.env, 
        #                     path=args.path, 
        #                     policy=self.policy)
        ...
        # set model throught and latancy collection list
        self.latancy_dict = defaultdict(list)
        self.throughput_dict = defaultdict(list)
    ...
    def _model_call(self, inps, attn_mask=None, labels=None):
        ...
        with torch.no_grad():
            # return logits
            # print(f"input ids len is: {len(inps)} \n ========*******")
            # print(f"attention mask len is: {len(attn_mask)
            if attn_mask is not None or labels is not None:
                raise ValueError("Not Supported when self.AUTO_MODEL_CLASS == transformers.AutoModelForSeq2SeqLM")
                # return ValueError("attn_mask and labels is not supported if using AutoModelForSeq2SeqLM")
                ...
                latancy_throughput = self.aggregated_throughout_lantency(
                ...
                return logits, latancy_throughput
    ...
    def _model_generate(self, context, max_length, stop, **generation_kwargs):
        ...
        latancy_throughput = self.aggregated_throughout_lantency(
            ...
            prompt_len=prompt_len, 
            cut_gen_len=self.cut_gen_len, 
            gen_len=max_length-context.shape[1])

        return outputs, latancy_throughput
    ...
    def loglikelihood_rolling(
        ...
        if self.world_size > 1:
            mytensor = torch.tensor(len(all_windows), 
                                    device=self.device
                                    )
        ...
        for i in range(0, len(all_windows), batch_size):
            ...
            batch_nlls, latency_throughput = self._loglikelihood_tokens(
        ...
        return loglikelihoods, latency_throughput_list
    ...
    def _loglikelihood_tokens(
        ...
        for chunk in chunks:
            ...
            logits, latency_throughput = self._model_call(batched_inps, **call_kwargs)
        ...
        pbar.close()
        # throughout_latency = self.compute_latency_throughput(
        #             self.latancy_dict, 
        #             self.throughput_dict
        #         )
        return re_ord.get_original(res), latency_throughput_list
    ...
    def generate_until(
        ...
        for chunk in chunks:
            ...
            cont, latency_throughput = self._model_generate(
                context=context_enc,
        ...
        return res, latency_throughput_list
    ...
    def aggregated_throughout_lantency(self, 
                                    time_costs, 
                                    num_prompts,
                                    prompt_len,
                                    cut_gen_len,
        ...
        prefill_latency = time_costs[0]
        prefill_throughput = num_prompts * prompt_len / prefill_latency
        if cut_gen_len:  # project latency of cut_gen_len to gen_len
            decode_latency = project_decode_latency(time_costs, prompt_len, gen_len)
        else:
            decode_latency = sum(time_costs[1:])
        ...
        return [prefill_latency, decode_latency, total_latency, prefill_throughput, decode_throughput, total_throughput]
        self.latancy_dict["prefill"].append(prefill_latency)
        self.latancy_dict["decode"].append(decode_latency)
        self.latancy_dict["total"].append(total_latency)
        self.throughput_dict["prefill"].append(prefill_throughput)
        self.throughput_dict["decode"].append(decode_throughput)
        self.throughput_dict["total"].append(total_throughput)
    
    
        # return prefill_latency, prefill_throughput, decode_latency, decode_throughput, total_latency, total_throughput
