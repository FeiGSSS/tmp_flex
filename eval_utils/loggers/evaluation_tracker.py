...
# import utils as logger_utils
# import sys
# from pathlib import Path
# dir_name = Path(__file__).parent.parent.parent.resolve()
# sys.path.insert(0, str(dir_name))
        ...
        if self.output_path:
            try:
                ...
                try:
                    ...
                    eval_logger.error(
                        "Failed to update general config tracker with asdict method"
                    )
                    eval_logger.info(f"general_config_tracker: {self.general_config_tracker.__dict__}")
                dumped = json.dumps(
                    results,
                    indent=2,
