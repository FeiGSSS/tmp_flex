...
from eval_utils.api.group import ConfigurableGroup, GroupConfig
from eval_utils.api.task import ConfigurableTask, Task
from eval_utils.evaluator_utils import get_subtask_list
...
class TaskManager:
    ...
    def __init__(
        ...
        self._task_index = self.initialize_tasks(
            include_path=include_path, include_defaults=include_defaults, data_path=data_path
        )
    ...
    def initialize_tasks(
        ...
        """
        if include_defaults:
            all_paths = [os.path.dirname(os.path.abspath(__file__)) + "/"] if data_path is None else [os.path.dirname(os.path.abspath(data_path)) + "/"]
        else:
            all_paths = []
...
def _check_duplicates(task_dict: dict) -> None:
    ...
    Takes the output of lm_eval.eval_utils.evaluator_utils.get_subtask_list and
